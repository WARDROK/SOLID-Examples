...
public:
    std::string getInfo() const override {
        return "Print & Fax Device";
    }
};

int main() {
    std::unique_ptr<PrintScanDevice> classicPrinter = std::make_unique<PrintScanDevice>();
    std::cout << classicPrinter->getInfo() << std::endl;
    classicPrinter->print("Document A");
    classicPrinter->scan("Document A");

    std::unique_ptr<PrintFaxDevice> classicFax = std::make_unique<PrintFaxDevice>();
